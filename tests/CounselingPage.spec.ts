...
test.describe("homeroom teacher", () => {
  ...
  test("첫 번째 상담 글 클릭 시 /write로 이동하고 상세 내용이 표시된다", async ({
    page,
  }) => {
    await page.locator('[data-testid="student-list"] tbody tr').first().click();
  ...
  test("취소 버튼 클릭 시 이전 페이지(/counseling)로 돌아가는지 확인", async ({
    page,
  }) => {
    // 1) 글 선택
    await page.locator('[data-testid="student-list"] tbody tr').first().click();
...
test.describe("teacher", () => {
  ...
  test("상담 내역 테이블이 제대로 렌더링된다", async ({ page }) => {
    ...
    const first = rows.first();
    await expect(first.getByTestId("counseling-cell-id")).toHaveText("1");
    await expect(first.getByTestId("counseling-cell-title")).toHaveText("123");
    await expect(first.getByTestId("counseling-cell-author")).toHaveText(
      "박서윤"
    );
    await expect(first.getByTestId("counseling-cell-subject")).toHaveText(
  ...
  test("기간 유형으로 검색 시 해당 날짜(2025-05-04)만 노출되는지 확인", async ({
    page,
  }) => {
    const studentSearchInput = page.getByTestId("student-search-input");
    await expect(studentSearchInput).toBeVisible();
    ...
    const dateInput = page.getByTestId("counseling-search-date");
    await expect(dateInput).toBeVisible();
    await dateInput.fill("2025-05-04");
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      const formatted = `${y}-${String(m).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
      expect(formatted).toBe("2025-05-04");
    }
  });
